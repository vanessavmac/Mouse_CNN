...
def gen_anatomy(data, input_depths = ['4'],
    ...
    # create layer 0 (LGNd, sSC)
    # NOTE: RGCs are not included in the anatomical net because they are modeled separately in retina.py
    ...
    LP = AnatomicalLayer('LP', '', data.get_num_neurons('LP', None))
    anet.add_layer(LGNd)
    anet.add_layer(sSC)
    anet.add_layer(LP)
    output_map[0] = [LGNd, sSC, LP]
